  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (startDateTime && endDateTime && terminalSerialNumber && type && executeOn) {
        ...
        const scheduleData = await callSetting.create({ startDateTime, endDateTime, terminalSerialNumber, type, file, executeOn });
        if (scheduleData) {
          logger.debugOut(
            __filename,
            { callSchedule },
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (query) {
        ...
        let heartbeatScheduledFile = { status: true, successCode: 'HEART_RESPONSE' };

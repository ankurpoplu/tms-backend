  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (type === FILESTORE.BLACKLIST) {
        ...
        name = 'V' + `${latestCreated ? latestCreated.version : version}` + '-' + `${type}` + '-' + path.parse(file.name).base;
        savePath = path.join(__dirname, '..' + `${process.env.UPLOAD_PATH}`, `${type}`, 'v_' + `${latestCreated ? latestCreated.version : version}`);
      } else {
